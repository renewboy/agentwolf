# 产品

AgentWolf 是一个本地优先的观战平台,运行由长驻 ACP Agent Session 进行对弈的狼人杀对局。
人类用户配置 Agent 工具、玩家 Profiles、Characters 与 board,然后通过 visibility-safe 的视图
观看一场由 server 主持的 Match。

源码派生的[游戏目录](generated/game-catalog.md)列出已安装的 Roles 与内置 boards。本文档拥有
用户工作流与可观察行为,不包含目录条目或规则实现。

## 配置

Agent 设置页管理 Agent Tools 与有序的 Agent Profiles。选择一个 Tool 会打开临时 ACP Session
以发现其宣告的模型。选择一个模型会刷新其支持的 reasoning 取值。Profile 存储 Tool、模型、可选
的 reasoning 强度与非机密连接设置;凭据来自命名的环境变量。

收藏页管理公开的 Character 卡。内置卡只读,可复制为可编辑的自定义卡。卡片控制背景、性格、
公开推理表现、发言风格、演绎边界与头像。它只改变表达:每个 Agent 保留完整的推理质量与最强
的对局判断。

Board 管理将只读内置 boards 与 6–24 席自定义 boards 相结合。自定义 board 存储完整身份牌池、
零至两张底牌、sheriff 与胜负政策,以及可选的逐 Seat Agent Profile 与 Character 默认值。实际席位
数量等于身份牌数量减去底牌数量;包含 Thief 时使用两张底牌。内置 boards 可先复制再编辑。

新建 Match 流程选择一个兼容 board,并按显式覆盖、board 默认值、再到首个有序 Profile 的顺序
解析每个 Seat。Profiles 与 Characters 可跨 Seat 复用。选择 Character 会以其 Character 名作为
昵称建议,昵称仍可编辑且必须唯一。开始 Match 会冻结 board、政策、Profile 选择、Character 卡、
昵称以及当前的全局发言长度指引。手动身份模式同时指定每个 Seat 和每张底牌,两类卡槽共同组成
board 冻结的完整身份牌池。
公开发言 interrupt 模式同时冻结到 Match;它决定旁听者是否获得后台反应回合,恢复时不会随 server
启动默认值变化。

## 玩家环境

每个 Seat 收到一个仅用于游戏的 Agent 环境,包含其玩家契约、所选 board、可见 Match 事实、玩家
Skills、本地读取/搜索工具、结构化游戏动作与赛后复盘动作。它不会收到无关的用户记忆、仓库开发
指令、浏览器/搜索权限、变更类工具、插件、hooks 或子代理。

每个 Seat 为 Match 创建一个逻辑 ACP Session。进程重启与传输恢复会续用同一个 Session ID。
Agent 收到的是增量可见事实,而非重复的完整历史。

## 对局体验

Match 界面以中央实时信息流为核心,两侧是玩家栏。信息流流式呈现 Agent 发言、公开事件、可见性
允许的夜晚信息、投票结果与赛后感想。对局历史按游戏时段折叠,并独立于固定舞台滚动。

用户可以在以下视图之间切换:

- 上帝视角,包含全部游戏 Roles 与私密游戏动作。
- 闭眼视角,只包含公开宣告的裁判事实。
- 玩家视角,包含某位玩家的公开事实、私密 Role 知情、阵营知情与私密结果。

Character 名与头像在每个视图中保持公开,不泄露隐藏的游戏 Role。淘汰与放逐在 Match 进行期间
保持普通身份隐藏;Role 特定的公开揭示仍属于其规则的一部分。终局身份事件在游戏结果之后揭示
每个 Seat;存在底牌时同时揭示底牌和已完成的身份转换来源。

结构化动作的就绪状态只在允许看到该玩家运行时状态的投影中显示。并行投票与动作在所有有资格
的玩家完成之前保持密封,因此响应顺序不会泄露选择。投票卡在每个目标下分组投票 Seat,并保留
弃票、权重与特定可见性的私密票。

警长和白天放逐的首轮平票都会进入平票玩家发言与复投。警长复投仍平票时本局失去警徽;放逐
复投仍平票时本轮无人出局并进入夜晚。

每个稳定的死亡、技能与遗言边界都会判定正式胜负。正式条件尚未发生时,若狼队仅凭自己已经
获得的可见事实便能在当前 board 的所有兼容身份与合法反制下锁定同一狼人结果,Match 直接进入
终局。本局采用狼刀在先规则。无法证明的局面继续运行;Village 与第三方阵营只按各自正式胜负
条件结束。

## 发言与特效

Agent 发言在流式传输时即呈现。提交事件使用同一段规范文本,将已知内部 Player ID 改写为公开
引用,并拒绝未知内部 ID 以便修正。玩家可以虚张声势地谈论身份与私密判断,但不能改写固定的
裁判事实。

语音开关在同一浏览器中保留,并在 Match 页面、同场轨迹页面、刷新与后续 Match 中沿用。一个已连接
的观战者控制当前 Match 的实时语音；完整句子形成后排队生成并流式播放,提交只补充最后的尾部。跳过作用于当前
玩家的整段发言,后续增量不会再次出声。一个阶段内的最后一段发言会持有后续阶段,直到播放完成、
被跳过、失败或控制者断开。

每条已提交发言都可以单独播放和停止。用户选择一条消息时,该消息立即取得音频焦点；期间出现的
现场发言保持文字呈现并继续推进,不抢占也不补播。语音开关关闭后仍可单独播放消息。对局页与同场
轨迹页之间切换保持当前声音连续,轨迹页不提供额外语音控件。

十二位内置 Character 使用各自的中文参考音色,角色语音按原速播放。服务启动后会在后台准备模型；
模型未就绪、未启动或音色缺失时，当前发言使用默认在线语音；后续发言自动优先使用已就绪的角色音色。
对应发言的播放键下方以小字显示回退原因，用户可点击“不再提示”并保存到当前浏览器；播放失败仍显示错误。用户通过语音
开关或单条消息播放按钮启用声音；浏览器需要用户交互才能播放时会显示操作提示。

只能播放当前视角可见的发言。切换视角会停止当前声音,后续只播放新视角可见的内容。生成或传输
中断会停止当前声音并保留文字；自动播报跳过该段,单条消息播放显示失败。

rolling 公开发言 interrupt 模式会在发言阶段入口和新的他人发言提交后,为具备 Phase 声明能力的
玩家启动后台增量判断。下一位发言者与刚完成发言的玩家不进入该次旁听;刚发言者在听到另一名玩家的
新发言后才重新获得判断。场上发言不等待这些判断;旧判断被取消后,同一 Session 只收到尚未确认的公开事实。
合法 interrupt 先完成时,当前发言停止,已经流出的干净文本保留在信息流中,随后执行 Role 自有的公开效果与阶段跳转。
该入口在每个新白天重新建立,支持狼人按夜间计划连续自爆。

## 赛后复盘

结果与终局身份揭示之后,server 发起一个十秒倒计时,提供立即开始或仅倒计时跳过。倒计时结束
自动开始复盘;已开始的复盘不可跳过。

每位原始玩家 Session 从获胜玩家中提交一位 MVP 提名,从其余玩家中提交一位 SVP 提名,并为其他
每个 Seat 打出五项评分。完成的评分表立即可见,且不进入未完成者的 Prompt。全部评分表到达后,
server 公布算术平均分与确定性奖项。

随后玩家按 Seat 顺序通过常规的流式发言气泡与播报路径进行感想陈述。完成或跳过复盘会关闭原始
Sessions,并把全部观战视角冻结为只读 Match 记录。历史记录仍可查看、切换视角与删除,但不能继续
游戏、重新运行复盘或生成仿真候选。复盘期间重复的传输失败会暂停对局,并提供操作者可见的继续动作。

## 开发者工作流

`pnpm dev:developer` 启用轨迹诊断,同时保留与普通模式相同的本地采集记录。监听地址由启动配置
决定。一条 Match 记录可打开以 Seat 为先的轨迹、Player/Record 检查器与语义审计问题。

同一 developer mode 提供独立的仿真工作流。尚未归档的暂停 Match 可以创建脱敏候选;浏览器工作流
审查、校验、确认警告并批准一个确定性 fixture。等效的 CLI 命令可用于自动化。采集与批准绝不修改
源 Match。

## 失败行为

schema 或规则非法的结构化动作在当前 Agent 回合内返回错误,允许在不改变游戏状态的前提下
重新调用。单次不确定的传输失败只针对受影响的玩家尝试同 Session 续接;重复或恢复失败会暂停
Match。

短暂的观战者断线保留最后有效投影,并以有界退避重连。暂停的 Match 在对局页提供继续和暂停详情，
删除操作位于大厅的对局管理入口。删除会移除该 Match 的持久记录与玩家 workspaces。未知或已删除的 Match 被判定为不可用,
不重试。
