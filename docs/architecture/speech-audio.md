...
校验期间持续显示，下载进度来自真实接收字节。加载阶段通过发言播放键下方的小字提示。
页面观察到实际下载后，在模型就绪时显示可关闭的完成通知；
...
自动队列把推理完成与播放完成分开。Core 发布当前及后续句段的有序快照，浏览器按单元标识维护
独立生成队列；每次收到完整音频 EOF 后立即请求下一句，播放队列继续输出此前已生成的声音。
预生成内容仍逐请求经过 server 可见性与原文校验。浏览器设置未消费音频容量上限，在数据被消费后
释放背压；队列撤销或视角变化会取消对应请求并释放缓冲。只有实际播放完成才通知呈现控制器。
