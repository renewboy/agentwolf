...
Simulation capture reads the immutable Match board, append-only event log, normalized player Turns,
accepted actions, and minimal playback-control records. Its replay payload replaces Match, Profile,
board, Session, delivery, name, time, and path identifiers; local candidate provenance retains only
the source Match ID and capture time. Raw Prompts, reasoning, tool output, credentials, diagnostics,
and runtime paths do not enter simulation fixtures.
