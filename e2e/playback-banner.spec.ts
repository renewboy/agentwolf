...
test('shows the next automatic portrait after closing the previous playback', async ({ page }) => {
  ...
  await playbackBar(page).getByRole('button', { name: '关闭', exact: true }).click()
...
for (const viewport of [
  ...
  test(`keeps playback controls inside the postgame status panel at ${viewport.width}px and reopens a replay`, async ({
    page,
  }, info) => {
    ...
    await bar.getByRole('button', { name: '关闭', exact: true }).click()
