...
test('streams a normalized developer trajectory with system instructions, prompt, reasoning, tool, and usage details', async ({
  ...
  await page
    .locator('.aw-match-hud')
    .screenshot({ path: testInfo.outputPath('match-header-centered.png') })
  await expect(page.getByRole('region', { name: '对局已暂停' })).toBeVisible()
  await expect(page.getByRole('link', { name: '切换到玩家行动轨迹' })).toBeVisible()
  ...
  const trajectoryNavigation = page.locator('.aw-match-navigation')
  const trajectoryBack = trajectoryNavigation.getByRole('link', { name: '返回大厅' })
  await expect(trajectoryBack).toBeVisible()
  await expect(page.locator('.aw-match-hud')).toHaveCount(1)
  await expect(page.locator('.aw-app-shell')).toHaveCount(0)
  await expect(page.getByRole('heading', { name: '玩家行动轨迹' })).toBeVisible()
  await expectDayCentered(page)
  await page
    .locator('.aw-match-hud')
    .screenshot({ path: testInfo.outputPath('trajectory-header-centered.png') })
  ...
  const narrowTrajectoryBounds = await page.locator('.aw-trajectory-layout').evaluate((layout) => {
    const inspector = layout.querySelector<HTMLElement>('.aw-trajectory-inspector')!
    const owners = [...layout.querySelectorAll<HTMLElement>('.aw-trajectory-owner')]
    return {
      ...
      ownerGaps: owners
        ...
        .map(
          (owner, index) =>
            owner.getBoundingClientRect().top - owners[index]!.getBoundingClientRect().bottom,
        ),
    }
  })
  expect(narrowTrajectoryBounds.inspectorRight).toBeLessThanOrEqual(narrowTrajectoryBounds.right)
  expect(narrowTrajectoryBounds.ownerGaps.every((gap) => gap >= 9)).toBe(true)
  await page.setViewportSize({ width: 2048, height: 1080 })
