...
PCM 排程与音量采样由 Core browser adapters 持有；产品 port 持有语音 HTTP 请求、MP3 流式播放、
Character 映射、字幕分页和提示。当前输出通过 Core `output` 快照关联稳定发言标识与玩家，
立绘只在实际起播后展示，字幕依次呈现当前片段与下一片段，缓冲时保留当前字幕。下一片段来自
port 音频分页或 Core 中同次发言的后续队列，流式追加时更新。立绘呈现器消费独立的 assets 定义，左右站位来自
玩家列分组；播放控制行位于现有状态面板内部。立绘开关以 `agentwolf.portrait-enabled` 保存在浏览器，
默认开启，后续播放和页面刷新遵循该偏好；历史定位只临时隐藏当前立绘。手动重播生成新的播放标识。
...
MP3 播放可在没有 Web Audio 的浏览器中运行；Web Audio 可用时采样实际音量驱动口型。
