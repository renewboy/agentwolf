...
`EndgameRoleModel`。plugin 模型同时声明 `knowledgeAbilityIds`,并以获授权事件为来源贡献控制组
成员得到的身份观察。Ruleset 构建验证 material 与 information ability IDs 完整对应模型,并拒绝
缺失观察器、重复或引用未知 Role 的声明。加入夜间 batch 的 ability 还声明
`nightResolutionStage`;保护与狼刀进入
...
狼人求解器从最近的阵营名册、公开事件与控制组成员私有可见事件构造观察,并以 Player ID 为键从
冻结 board 牌池展开所有兼容隐藏 Role 世界。共享阵营知识的狼队形成一个控制组;Awakened Hidden
Wolf 只有在自己的已知能力允许时形成单人控制组。学习结果与自己执行的复制查验只收窄该单人
belief,不会进入普通狼队观察。Cupid 与 Thief plugin 分别约束关系可见性和有效身份牌池。第三方
目标存在、关系未知、底牌选择不可见或赢家集合不能保持一致时,证明不成立。
...
每个控制组采用对不可区分状态一致的目标策略,同一 belief 中只能选择全部兼容世界都合法的同一
Player ID。公开结果把后继世界按控制组可见观察重新分区。白天搜索区分首轮投票、平票 PK 复投和
复投平票流局;夜间搜索遵循同一狼刀检查点。其他玩家的投票、Witch 药物、Guard 保护、Hunter
开枪、Idiot 放逐免疫、Sheriff 权重与当前 Role 模型作为全称反制。规范化状态以 memoization 求解;
...
- 隐藏身份只扩大 belief 集合;只有控制组实际收到的身份观察可以把具体 Player ID 从兼容世界中
  确定下来,无法证明的复杂局面保持原流程。
